...
class CashVoucher(models.Model):
    ...
    def confirm_post(self):
        ...
        if self.amount != self.total:
            raise ValidationError(_('Total amount is not equal total expenses'))
        ...
        if self.amount == self.total:
            ...
            if self.currency_id != self.env.user.company_id.currency_id:
                # amount = amount+i.amount
                credit_amount = self.total / self.currency_id.rate
                currency_id = self.currency_id.id
                credit_curr_amount = credit_amount
            if self.currency_id == self.env.user.company_id.currency_id:
                # amount = amount + i.amount
            ...
            credit_val = {
                ...
                'credit': credit_curr_amount,
                'currency_id': currency_id,
                'partner_id': False,
                'company_id': self.company_id.id or False,
                'amount_currency': self.total * -1,
            }
    ...
    def confirm_manager(self):
        ...
        list = []
        list2 = []
        if self.is_transfer == False and self.is_sending_to_related_party == False:
